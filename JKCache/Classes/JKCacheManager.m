...
@interface JKCacheManager()
{
    ...
    JKFileCache *cacheFile;
}
@end

@implementation JKCacheManager

NSString *const CatalogFileName = @"durian_catalog";

static JKCacheManager *managerInstance;

+ (void)load
{
    JKCacheManager *manager = [JKCacheManager shareInstance];
    manager->cacheFile = [JKFileCache cacheWithName:CatalogFileName serialize:^NSData * _Nonnull(id<NSCoding>  _Nullable object, NSString * _Nullable key) {
        return  [NSJSONSerialization dataWithJSONObject:object options:NSJSONWritingPrettyPrinted error:nil];
    } deserialize:^id<NSCoding> _Nonnull(NSData * _Nullable data, NSString * _Nullable key) {
...
- (void)registFileInfo:(NSString *)name  fileSize:(NSUInteger)size withTimeLimit:(NSTimeInterval)timeLimit
{
    ...
    [self->cacheFile cacheObject:[self->catalogDict copy] withTimeLimit:NSIntegerMax cacheResult:nil];
    pthread_mutex_unlock(&_lock);
}
...
- (void)trimCatalog
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        [self->cacheFile object:^(id object, NSString *key) {
            if (object) {
