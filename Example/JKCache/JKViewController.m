    ...
    NSThread *t = [[NSThread alloc] initWithBlock:^{
        for (int i = 0; i < 1000; i++) {
            ...
            dispatch_async(concurrentQueue, ^{
                ...
                [[JKCache shareInstance] cacheObject:testModel forKey:[NSString stringWithFormat:@"test1%d", i] andTiemLimit:1 whenResult:^(BOOL isSuccess) {
                    NSLog(@"--------------");
                }];
            });
            ...
            dispatch_async(concurrentQueue, ^{
                ...
                [[JKCache shareInstance] cacheObject:obj forKey:[NSString stringWithFormat:@"test2%d", i] andTiemLimit:2 whenResult:^(BOOL isSuccess) {
                    NSLog(@"---------------2：%d", isSuccess);
                }];
            });
            dispatch_async(concurrentQueue, ^{
                ...
                [[JKCache shareInstance] cacheObject:obj forKey:[NSString stringWithFormat:@"test3%d", i] andTiemLimit:3 whenResult:^(BOOL isSuccess) {
                    NSLog(@"---------------3：%d", isSuccess);
                }];
            });
        }
...
{
    [[JKCache shareInstance] deleteObjectForKey:@"test1" whenResul:^(BOOL isSuccess, NSError *error) {
    ...
    [[JKCache shareInstance] deleteObjectForKey:@"test3" whenResul:^(BOOL isSuccess, NSError *error) {
        NSLog(@"---isSuccess:%d, eroor:%@", isSuccess, error);
    }];
}
...
- (IBAction)clickPrintButton:(id)sender
{
    ...
    for (int i = 0; i < 1; i++) {
        dispatch_async(concurrentQueue, ^{
            [[JKCache shareInstance] objectForKey:@"test11456" whenResult:^(id object, NSString *key) {
                NSLog(@"---key:%@, value:%@", key, object);
            }];
        });

        dispatch_async(concurrentQueue, ^{
            [[JKCache shareInstance] objectForKey:@"test29856" whenResult:^(id object, NSString *key) {
                NSLog(@"---key:%@, value:%@", key, object);
            }];
        });
        dispatch_async(concurrentQueue, ^{
            id object = [[JKCache shareInstance] objectForKey:@"test31"];
